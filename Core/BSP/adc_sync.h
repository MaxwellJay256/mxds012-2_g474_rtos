...
#define ADC_RESULT_BUFFER_LENGTH 400 // 每个 ADC 通道单轮连续采样点数
...
uint16_t ADC_Sync_GetSampleDepth(void);
uint8_t ADC_Sync_SetSampleDepth(uint16_t depth);
uint16_t ADC_Sync_GetScanFrequencyHz(void);
uint8_t ADC_Sync_SetScanFrequencyHz(uint16_t freq_hz);
